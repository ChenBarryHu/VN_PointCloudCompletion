...
class VN_DGCNN_fps(nn.Module):
    def __init__(self, config, only_coarse=False):
        super().__init__()
        '''
        K has to be 16
        '''
    ...
    def forward(self, x):
        ...
        f = self.vn_get_graph_feature(f_q, x_coord=coor_q)
        # f = self.conv3(f)
        f = self.conv4(f)
        f = self.pool2(f)


        f = self.vn_get_graph_feature(f, x_coord=coor_q)
        ...
        global_feature = self.pool5(f).unsqueeze(-1)
        coarse = self.conv7(global_feature).squeeze(-1)

        if self.num_coarse == 224:
            inp_sparse = fps(xyz, 224)
            coarse_cat = torch.cat([coarse, inp_sparse], dim=1).contiguous()
            return (coarse.contiguous(), coarse_cat.contiguous()), global_feature

        return coarse.contiguous(), global_feature
